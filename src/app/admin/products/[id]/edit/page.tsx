...
export default function EditProductPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    async function load() {
      ...
      const [prodRes, itemsRes] = await Promise.all([
        supabase.from('products').select('*').eq('id', id).eq('seller_id', session.user.id).single(),
        supabase.from('product_items').select('*').eq('product_id', id).order('order_index'),
      ])

      if (!prodRes.data) { router.replace('/admin/products'); return }

      const p = prodRes.data
      setTitle(p.title ?? '')
      setDescription(p.description ?? '')
      setType(p.type ?? 'course')
      setPrice(String(p.price_usd ?? ''))
      ...
      setItems((itemsRes.data ?? []).map(i => ({
        ...
        type:             i.type ?? 'video',
        url:              i.url ?? '',
        is_preview:       i.is_preview ?? false,
        duration_seconds: i.duration_seconds ? String(i.duration_seconds) : '',
      })))
  ...
  function addItem() {
    setItems(prev => [...prev, { title: '', type: 'video', url: '', is_preview: false, duration_seconds: '' }])
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // Eliminar items existentes y re-insertar (más simple que diff)
      ...
      console.error(err)
      setError('Error guardando los cambios. Revisá los datos.')
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <div className="max-w-2xl">
      <div className="mb-6">
        ...
        <h1 className="text-2xl font-black text-white" style={{ letterSpacing: '-0.02em' }}>Editar producto</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-5">
        ...
        <div>
          ...
          {items.length === 0 && (
            <p className="text-sm text-center py-6 rounded-xl"
              ...
              No hay contenido. Hacé click en "Agregar" para añadir clases o archivos.
            </p>
          )}
        ...
        <div className="flex items-center gap-3 p-4 rounded-xl"
          ...
          <div className="flex-1">
            <p className="text-white font-semibold text-sm">Publicado</p>
