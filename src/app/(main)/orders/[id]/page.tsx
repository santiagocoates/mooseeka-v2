...
import {
  ...
  Download, Clock, Shield, MessageSquare, Star
} from 'lucide-react'
...
interface OrderData {
  ...
  buyer_id: string
  seller_id: string
  service: { title: string; cover_url: string | null; delivery_days: number | null; revisions: number | null }
  seller: { name: string; username: string; avatar_url: string | null }
...
const STATUS_CONFIG: Record<OrderStatus, { label: string; color: string; bg: string; icon: React.ReactNode; description: string }> = {
  ...
  cancelled:   { label: 'Cancelado',   color: '#6B7280', bg: 'rgba(107,114,128,0.1)',  icon: <AlertTriangle size={14} />, description: 'Este pedido fue cancelado.' },
}
...
export default function OrderDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const fetchOrder = useCallback(async () => {
    ...
    const { data, error } = await supabase
      .from('orders')
      .select(`
        id, status, project_name, notes, deadline,
        price, currency, revisions_used, created_at, completed_at,
        buyer_id, seller_id,
        service:services(title, cover_url, delivery_days, revisions),
        seller:profiles!orders_seller_id_fkey(name, username, avatar_url),
  ...
  async function updateStatus(newStatus: OrderStatus, extra: Record<string, unknown> = {}) {
    if (!order) return
    setUpdating(true)
    const supabase = createClient()
    const { error } = await supabase
      ...
      .eq('id', order.id)
    if (!error) await fetchOrder()
    setUpdating(false)
  }

  async function handleApprove() {
    await updateStatus('completed', { completed_at: new Date().toISOString(), escrow_released: true })
    setShowReview(true)
  }

  async function handleRequestRevision() {
    if (!order) return
    await updateStatus('revision', { revisions_used: order.revisions_used + 1 })
  ...
  async function handleSellerAction(newStatus: 'in_progress' | 'delivered') {
  ...
  if (notFound || !order) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen gap-3">
        ...
        <p className="text-white font-bold">Pedido no encontrado</p>
        <Link href="/orders" className="text-sm" style={{ color: '#8B3FFF' }}>Volver a Mis pedidos</Link>
      </div>
    )
  }
  ...
  const maxRevisions = order.service?.revisions === 99 ? Infinity : (order.service?.revisions ?? 2)
  const canApprove = isBuyer && order.status === 'delivered'
  const canRevise = isBuyer && order.status === 'delivered' && order.revisions_used < maxRevisions
  const priceLabel = `${order.currency === 'EUR' ? '€' : '$'}${order.price}`
  const delivDays = order.service?.delivery_days
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 md:px-6 py-6">
      ...
      <div className="flex items-start justify-between mb-6 flex-wrap gap-3">
        <div>
          <p className="text-xs mb-1" style={{ color: '#7A6890' }}>Pedido #{order.id.slice(0, 8)}</p>
          <h1 className="text-white text-xl font-black" style={{ letterSpacing: '-0.02em' }}>
            {order.service?.title ?? order.project_name}
          </h1>
          <p className="text-sm mt-1" style={{ color: '#7A6890' }}>
            {formatDate(order.created_at)} · {priceLabel} Por proyecto
          </p>
        </div>
      ...
      <div className="flex flex-col md:flex-row gap-6">
          ...
          <div className="rounded-2xl p-5"
            ...
            <div className="flex flex-col gap-3">
              <div>
                ...
                <p className="text-white text-sm font-semibold">{order.project_name}</p>
              </div>
              {order.notes && (
                <div>
              ...
              <div className="grid grid-cols-2 gap-3 pt-1">
                {[
                  { label: 'Tiempo de entrega', value: delivLabel },
                  { label: 'Revisiones', value: `${order.revisions_used}/${order.service?.revisions === 99 ? '∞' : (order.service?.revisions ?? 2)} usadas` },
                ].map(d => (
              ...
              {order.deadline && (
                <div>
                  <p className="text-xs font-bold uppercase tracking-wider mb-1" style={{ color: '#7A6890' }}>Fecha límite</p>
                  <p className="text-white text-sm">{new Date(order.deadline).toLocaleDateString('es-ES', { day: 'numeric', month: 'long', year: 'numeric' })}</p>
                </div>
              )}
            </div>
          </div>
            ...
            <div className="rounded-2xl p-5"
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  disabled={revisionNote.trim().length < 10 || updating}
        ...
        <div className="w-full md:w-56 shrink-0 flex flex-col gap-4">
          ...
          {canApprove && (
            <div className="rounded-2xl p-4 flex flex-col gap-2"
              ...
              <button
                ...
                style={{ background: 'linear-gradient(135deg,#8B3FFF,#FF1A8C)' }}>
                <Check size={14} /> {updating ? 'Procesando...' : 'Aprobar entrega'}
              </button>
              ...
              {canRevise && (
                <button
                  ...
                  disabled={updating}
                  className="w-full py-2.5 rounded-xl text-sm font-semibold flex items-center justify-center gap-2 transition-all hover:opacity-80 disabled:opacity-60"
              ...
              <button
                ...
                disabled={updating}
                className="w-full py-2.5 rounded-xl text-xs font-medium flex items-center justify-center gap-2 transition-all hover:opacity-70 disabled:opacity-60"
                style={{ color: '#EF4444' }}>
          ...
          {/* Seller actions */}
          {isSeller && (order.status === 'pending' || order.status === 'in_progress' || order.status === 'revision') && (
            <div className="rounded-2xl p-4 flex flex-col gap-2"
              style={{ background: 'rgba(14,0,28,0.9)', border: '1px solid rgba(123,47,255,0.3)' }}>
              <p className="text-xs font-bold uppercase tracking-wider mb-1" style={{ color: '#7A6890' }}>Tu turno</p>

              {order.status === 'pending' && (
                <button
                  onClick={() => handleSellerAction('in_progress')}
                  disabled={updating}
                  className="w-full py-3 rounded-xl text-white font-bold text-sm flex items-center justify-center gap-2 transition-all hover:opacity-90 disabled:opacity-60"
                  style={{ background: 'linear-gradient(135deg,#8B3FFF,#FF1A8C)' }}>
                  <RefreshCw size={14} /> {updating ? 'Procesando...' : 'Comenzar trabajo'}
                </button>
              )}

              {(order.status === 'in_progress' || order.status === 'revision') && (
                <button
                  onClick={() => handleSellerAction('delivered')}
                  disabled={updating}
                  className="w-full py-3 rounded-xl text-white font-bold text-sm flex items-center justify-center gap-2 transition-all hover:opacity-90 disabled:opacity-60"
                  style={{ background: 'linear-gradient(135deg,#8B3FFF,#FF1A8C)' }}>
                  <Check size={14} /> {updating ? 'Procesando...' : 'Marcar como entregado'}
                </button>
              )}
            </div>
          )}
          ...
          <div className="rounded-2xl p-4"
            ...
            <Link href={`/${order.seller.username}`} className="flex items-center gap-2.5 mb-3 hover:opacity-80 transition-opacity">
              {order.seller.avatar_url ? (
                <img src={order.seller.avatar_url} alt={order.seller.name}
          ...
          {order.status !== 'completed' && order.status !== 'cancelled' && (
            <div className="rounded-2xl p-4"
              ...
              <p className="text-[11px] leading-relaxed" style={{ color: '#7A6890' }}>
                {priceLabel} retenido en escrow. Se libera al vendedor cuando apruebes la entrega.
              </p>
            </div>
          )}
        </div>
